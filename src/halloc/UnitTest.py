...
GcovPath="./" #覆盖率文件路径(一般不用修改)
FilePath="../" #待编译文件以及编译后生成的文件所在的路径(一般不用修改)
GccFile = ["errno/err.c", "except/except.c", "list/list.c", "timer/timer.c", "halloc/system_layer.c", "halloc/manager_layer.c", "halloc/alloc_layer.c", 
"halloc/gc_layer.c", "thread/thread.c", "thread/mutex_cond.c", "halloc/test_halloc.c"] #编译关联文件
TestFile = ["system_layer.c"]; #待测试测试文件
#TestFile = ["halloc/manager_layer.c"]; #待测试测试文件
#TestFile = ["halloc/alloc_layer.c"]; #待测试测试文件
#/************************************************************/
...
try:
    try:
        ...
        HtmlCover = coverage.Coverage(GcovPath, UnitPath, common.ResultDir, common.Gcov, TestFile ) #生成覆盖率文件
